...
# 사용할 augmentation 을 import
from albumentations import (
    CenterCrop, ColorJitter, #ToTensor, Normalize,Resize,GridDropout
    ToGray, HorizontalFlip, ShiftScaleRotate, RandomBrightnessContrast, GaussNoise, CLAHE
)
...
class CustomAugmentation:
    def __init__(self, resize, mean, std, **args):
        self.transform = Compose([
            CenterCrop(height=224, width=224), # 가운데 자르기 + vit patch로 나누기.
            ColorJitter(0.1, 0.1, 0.1, 0.1), # 
            #GridDropout,
            ToGray(p=1.0), #색 정보 삭제, 1.0 확률로 적용
            # HorizontalFlip(p=0.5), #수평 뒤집기, 0.5 확률로 적용 
            # VerticalFlip(p=0.5), #수직 뒤집기, 0.5 확률로 적용
            ShiftScaleRotate(p=1.0), # 회전 및 확대 축소 
            RandomBrightnessContrast(brightness_limit=(-0.3, 0.3), contrast_limit=(-0.3, 0.3), p=0.5), # 밝기 및 대비 조정
            GaussNoise(var_limit=(1000, 1600), p=0.3), # 가우시안 노이즈
            CLAHE(p=0.3),
            # CenterCrop((320, 256)),
            # Resize(resize, Image.BILINEAR),
            # ColorJitter(0.1, 0.1, 0.1, 0.1),
...
class MaskBaseDataset(Dataset):
    ...
    num_classes = 3 + 2 + 43 # Multi Label Classification (NewAgeLAbel)
    ...
    def setup(self):
        ...
        for profile in profiles:
            ...
            for file_name in os.listdir(img_folder):
                ...
                self.labels.append(mask_label * 6 + gender_label * 3 + age_label) # Kfold
