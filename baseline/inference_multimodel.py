...
def inference(data_dir, model_dir, output_dir, args):
    ...
    mask_model = load_model(model_dir+'/mask', 3, device).to(device)
    mask_model.eval()
    gender_model = load_model(model_dir+'/gender', 2, device).to(device)
    gender_model.eval()
    age_model = load_model(model_dir+'/age', 3, device).to(device)
    age_model.eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', type=str, default=os.environ.get('SM_CHANNEL_MODEL', './model')) # change output dir
